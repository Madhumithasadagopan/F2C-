...
$sql="update tblvehicles set ProductTitle=:producttitle,Product=:product,ProductOverview=:productoverview,Price=:price,Location=:location,ValidDate=:validdate,PhoneNo=:phoneno,AirConditioner=:airconditioner,PowerDoorLocks=:powerdoorlocks,AntiLockBrakingSystem=:antilockbrakingsys,BrakeAssist=:brakeassist,PowerSteering=:powersteering,DriverAirbag=:driverairbag,PassengerAirbag=:passengerairbag,PowerWindows=:powerwindow,CDPlayer=:cdplayer,CentralLocking=:centrallocking,CrashSensor=:crashcensor,LeatherSeats=:leatherseats where id=:id ";
...
<head>
	...
	<title>FarmoCo portal| Admin Edit Product Info</title>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT tblvehicles.*,tblbrands.ProductName,tblbrands.id as bid from tblvehicles join tblbrands on tblbrands.id=tblvehicles.Product where tblvehicles.id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<option value="<?php echo htmlentities($results->Location);?>"> <?php echo htmlentities($result->Location);?> </option>

<option value="Tamilnadu">Chennai</option>
<option value="Kar">Hyderabad</option>
<option value="Coimbatore">Bangalore</option>
</select>
</div>
</div>
...
<div class="form-group">
<label class="col-sm-2 control-label">ValidDate<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="year" class="form-control" value="<?php echo htmlentities($result->ValidDate);?>" required>
</div>
<label class="col-sm-2 control-label">PhoneNo<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="validdate" class="form-control" value="<?php echo htmlentities($result->PhoneNo);?>" required>
</div>
</div>
<div class="hr-dashed"></div>								
<div class="form-group">
